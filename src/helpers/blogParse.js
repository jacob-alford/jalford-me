...
const getKatex = object => {
  ...
  return deepArr &&
    ...
    ? [
        [deepArr[0].substring(1), ...inSplice(deepArr, 0, 1)]
          .join('')
          .replace(/\s,\\,\\,\s/gm, ` \\\\ `),
        getKatexType(deepArr[0][0])
      ]
    : [false, false];
};

const inlineMath = str =>
  str &&
  typeof str === 'string' &&
  str
...
const Katex = props => {
  ...
  return katexStr === false || katexType === false ? (
    <span style={styles.strikethrough}>{props.children}</span>
  ) : (
    <span
      style={katexType === 'block' ? styles.math : null}
...
const markdownConfig = {
  ...
  delete: Katex,
  heading: props => (
    <AdaptiveHeading level={props.level}>{props.children}</AdaptiveHeading>
  ),
  image: props => (
    <span>
      <Image
      ...
      <List>
        {props.children.map((child, index) => {
          return React.cloneElement(child, {
            depth: props.depth,
            isChecklistItem: !checklistIndecies.includes(null)
          });
        })}
      </List>
    );
  ...
  listItem: props => {
    if (props.isChecklistItem)
      return (
        <ListItem>
          <ListItemIcon style={{ minWidth: 0, marginRight: '14px' }}>
            {props.checked ? <CheckCircleOutline /> : <RadioButtonUnchecked />}
          </ListItemIcon>
      ...
      return (
        <ListItem>
          <ListItemText>
            <span style={{ marginRight: '14px' }}>{props.index + 1}.</span>
            {props.children}
          </ListItemText>
        </ListItem>
      );
        ...
        <ListItem>
          <ListItemText>
            <span
              ...
              dangerouslySetInnerHTML={getDepthIcon(props.depth)}
            />
            {props.children}
          </ListItemText>
        </ListItem>
