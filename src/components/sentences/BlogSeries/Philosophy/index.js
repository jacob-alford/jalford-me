...
class randomBokehBall {
	constructor(width, height, index) {
		...
		this.maxOpacity = Math.random() * 0.5 + 0.25;
		this.currentFade = 0;
		this.containerHeight = height;
		this.containerWidth = width;
		this.direction = Math.random() < 0.009 ? -1 : 1;
...
const progressBall = (ball, bokeh) => {
	...
	if (ball.currentFade === 0 && ball.currentLinger >= ball.lingerDuration) {
		bokeh.splice(
			ball.index,
			1,
			new randomBokehBall(ball.containerWidth, ball.containerHeight, ball.index)
		);
		ball = null;
	} else {
		/* Fade / Grow In */
		...
		const draw = () => {
			...
			for (let ball of bokeh) {
				...
				context.ellipse(
					ball.x,
					ball.y,
					ball.radius * h2w * ball.currentGrow,
					ball.radius * w2h * ball.currentGrow,
					0,
					0,
					2 * Math.PI
				);
			...
			if (counter <= breakpointStagger) counter++;
		};
