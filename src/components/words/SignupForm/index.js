...
export default function SignupForm(props) {
	...
	function validForm() {
		return validateEmail(email) && passwordScore >= 2 && validateUsername(username);
	}
				...
				<Grid container alignItems='center' direction='row'>
					<Grid item>
						<Button
							disabled={!validForm()}
							style={
								validForm() ? { color: color, ...styles.submitButton } : styles.submitButton
							}
					...
					<Grid item>
						{loading ? <CircularProgress style={styles.loader} size={15} /> : null}
					</Grid>
				</Grid>
			</Grid>
		</Grid>
