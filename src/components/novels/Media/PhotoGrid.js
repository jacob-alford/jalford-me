...
const createArr = length => {
  const out = [];
  for (let i = 0; i < length; i++) out.push(null);
  return out;
};
...
export default function PhotoGrid(props) {
  ...
  if (divzBy4(photos.length)) return <GridBy4 photos={photos} />;
  else if (divzBy3(photos.length)) return <GridBy3 photos={photos} />;
  else return <GridBy2 photos={photos} />;
}
