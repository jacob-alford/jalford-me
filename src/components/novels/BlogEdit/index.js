...
const sameArr = (arr1, arr2) => {
  ...
  const greatestLength = wrkArr.length > wrkArr2.length ? wrkArr.length : wrkArr2.length;
  for (let i = 0; i < greatestLength; i++) {
    if (wrkArr[i] !== wrkArr2[i]) return false;
  }
  return true;
};
    ...
    if (data.postData) {
      if (data.postData.snapshots && data.postData.snapshots.length > 0) {
        if (data.postData.lastPublish) {
          if (
            data.postData.snapshots[0].date.seconds > data.postData.lastPublish.seconds
          ) {
          ...
          <React.Fragment>
            <Grid item>
              <Grid
                ...
                <Grid item>
                  <Button
                    disabled={!bodyHasChanged(true)}
                    onClick={handleSnap}
                    style={
                      bodyHasChanged(true)
                        ? styles.updateSnapshot
                        : styles.disabledUpdateSnapshot
                    }
                    variant='outlined'>
                    Snap
                  </Button>
                ...
                <Grid item>
                  ...
                  <Button
                    ...
                    style={somethingHasChanged() ? styles.save : styles.saveDisabled}
                    variant='outlined'>
                    Publish
                  </Button>
                </Grid>
                    ...
                    <Grid spacing={2} container direction='column'>
                      ...
                      <Grid item>
                        <TextField
                          type='datetime-local'
                          label='Date'
                          value={blogDate || new Date().toISOString()}
                      ...
                      <Grid item>
                        <FormControlLabel
                          control={
                            <Switch checked={isPublic} onChange={handleIsPublicToggle} />
                          }
                        ...
                        <FormControlLabel
                          control={
                            <Switch
                              ...
                              onChange={handleDisplayHeadingToggle}
                            />
                          }
                        ...
                        <Grid container direction='row' spacing={1}>
                          <Grid item>
                            <TextField
                              label='Add Tag'
                              value={addTagBox}
                              onChange={handleAddTagBoxChange}
                            />
                          </Grid>
                          <Grid item>
                            <Button
            ...
            <React.Fragment>
              {displayHeading ? (
                <React.Fragment>
                  <Typography
                    paragraph
                    style={{ textAlign: 'center', fontSize: titleSize }}
                    variant='h1'>
                    {blogTitle}
                  </Typography>
                  <Typography paragraph variant='h4' style={{ textAlign: 'center' }}>
                    ...
                    <strong>
                      {` ${new Date(blogDate).toLocaleDateString('default', {
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric'
                      })}`}
