...
const fields = [
  {
    field: 'Actions',
    icon: color => <Build style={{ ...styles.chipIcon, color: color }} />,
    label: 'Actions'
  },
  {
    field: 'title',
    icon: color => <Title style={{ ...styles.chipIcon, color: color }} />,
    label: 'Title'
  },
  {
    field: 'date',
    icon: color => <Event style={{ ...styles.chipIcon, color: color }} />,
    label: 'Date'
  },
  {
    field: 'isPublic',
    icon: color => <Public style={{ ...styles.chipIcon, color: color }} />,
    label: 'Public'
  },
  {
    field: 'uid',
  ...
  {
    field: 'author',
    icon: color => <Person style={{ ...styles.chipIcon, color: color }} />,
    label: 'Author'
  },
  {
    field: 'tags',
    icon: color => <Label style={{ ...styles.chipIcon, color: color }} />,
    label: 'Tags'
  },
  {
    field: 'lastPublish',
    icon: color => <Publish style={{ ...styles.chipIcon, color: color }} />,
    label: 'Last Published'
  },
  {
    field: 'series',
    icon: color => <Category style={{ ...styles.chipIcon, color: color }} />,
    label: 'Series'
  }
];

const dateify = date =>
  new Date(date.toDate()).toLocaleDateString('default', {
    year: 'numeric',
    month: 'long',
    day: 'numeric'
  });
  ...
  const headerConfig = [
    {
      ...
      transform: (ref, ref2) => (
        <Grid container justify='center' alignItems='center' style={styles.actionBar}>
          ...
          <Grid item>
            <IconButton
              style={currentPublic === ref ? { color: '#E84040' } : null}
              onClick={() => handleTogglePublic(ref, ref2)}>
              <Public />
            </IconButton>
          </Grid>
          <Grid item>
            <IconButton
              style={currentDelete === ref ? { color: '#E84040' } : null}
    ...
    },
    {
      ...
      ref: ['lastPublish'],
      sortable: true,
      transform: date => date && dateify(date)
    },
    {
      label: 'Date',
      ref: ['date'],
      sortable: true,
      transform: date => dateify(date)
    },
    {
      label: 'Public',
      ref: ['isPublic'],
      sortable: true,
      transform: bool => bool.toString()
    },
