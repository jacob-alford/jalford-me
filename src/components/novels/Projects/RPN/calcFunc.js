  ...
  return (stack, tape) => {
    try {
      ...
      if (typeof opString === 'string') {
        tapeMessage = [
          `${opString}(${displayNum(output)})`,
          `&#8195;= ${displayNum(output)}`
        ];
      } else {
        tapeMessage = [
          opString(op(displayNum(output))),
          `&#8195;= ${displayNum(op(output))}`
        ];
      }
      return [
...
function createDoubleOperator(op, opString) {
  return (stack, tape) => {
    try {
      ...
      if (typeof opString === 'string') {
        tapeMessage = [
          `${displayNum(stack[0])} ${opString} ${displayNum(stack[1])}`,
          `&#8195;= ${displayNum(op(stack[0], stack[1]))}`
        ];
      } else {
        tapeMessage = [
          opString(displayNum(stack[0]), displayNum(stack[1])),
          `&#8195;= ${displayNum(op(stack[0], stack[1]))}`
        ];
      }
      return [
        [output, ...stack.slice(2)],
        [...tapeMessage, ...tape]
      ];
    } catch (error) {
      console.error(`Invalid operation not caught before execution!  Error: ${error}`);
...
function createReducer(op, reduceMsg) {
  return (stack, tape) => {
    try {
      ...
      const tapeMessage = [
        `${reduceMsg}(${displayStackInline(stack)})`,
        `&#8195;= ${displayNum(output)}`
      ];
...
function createNumType(num) {
  return (stack, tape) => {
    ...
    if (stack[0].toString().includes('e')) {
      if (tape[0] && tape[0].toString().includes('ENTER')) newStack[0] = num;
      else {
        let temp = newStack[0].toString().split('');
        if (stack[0].toString().includes('.'))
          temp.splice(newStack[0].toString().indexOf('e'), 0, num);
        else temp.splice(newStack[0].toString().indexOf('e'), 0, `.${num}`);
        newStack[0] = Number(temp.join(''));
      }
    } else {
      if (tape[0] && tape[0].toString().includes('ENTER')) newStack[0] = num;
      else newStack[0] = Number(newStack[0].toString() + num);
    }
...
calcFunctions['type0'] = {
  fn: (stack, tape) => {
    const newStack = [...stack];
    if (stack[0].toString().includes('e')) {
      if (tape[0] && tape[0].toString().includes('ENTER')) newStack[0] = 0;
      else {
        let temp = newStack[0].toString().split('');
        if (stack[0].toString().includes('.'))
          temp.splice(newStack[0].toString().indexOf('e'), 0, `0`);
        else temp.splice(newStack[0].toString().indexOf('e'), 0, `.0`);
...
calcFunctions['dot'] = {
  fn: (stack, tape) => {
    if (stack[0].toString().includes('.') && !tape[0].toString().includes('ENTER'))
      return 'Unable to add a second decimal point!';
    const newStack = [...stack];
    if (stack[0].toString().includes('e')) {
      if (tape[0] && tape[0].toString().includes('ENTER')) newStack[0] = '0.';
      else {
      ...
      if (tape[0] && tape[0].toString().includes('ENTER')) newStack[0] = '0.';
      else newStack[0] = newStack[0].toString() + '.';
    }
    return [newStack, [`IGNORE`, ...tape]];
  },
  colorClass: 'number',
...
calcFunctions['swap'] = {
  fn: (stack, tape) => {
    try {
      ...
      return [
        [secondElement, firstElement, ...copyStack],
        [`SWAP(${displayNum(firstElement)},${displayNum(secondElement)})`, ...tape]
      ];
    } catch (error) {
      console.error(`Invalid operation not caught before execution!  Error: ${error}`);
      return error;
