...
const getRandomIncorrect = () => [
  'Not correct', 'That does not match',
  'Bad guess', 'Does not comprehend',
  'AI shutting down', 'Puzzle is NOT solved',
  'One or more symbols are incorrect', 'RED TEXT',
  'A PROBLEM IS BELOW', 'Try again, please',
  'Try again','>=1 Symbol wrong'
][(Math.random() * 12) | 0];
    ...
    if(hint1Valid && hint2Valid && hint3Valid && !notifyLock){
      try{
        const solution = AES.decrypt(
          getPuzzleCipher(puzzleData),`${hint1}${hint2}${hint3}`
        ).toString(UTF8Enc);
        if(Boolean(solution)){
          lockNotifications();
          setHeading(`*You did it!`);
    ...
    <Holder className={classes.superHolder}>
      ...
      <Holder justify="space-evenly" direction="row">
        {puzzleState.map((item,index) => (
          <Holder key={`puzzleItem${index}`} className={classes.letterHolder} style={{margin:(screenTooSmall) ? "0px" : null}}>
