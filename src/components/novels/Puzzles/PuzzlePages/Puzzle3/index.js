...
const setSuccessHeader = (test,hash,setter) => () => {
  if(test && hash && sha3Check(test,hash))
    setter(true);
  else setter(false);
}
...
function Puzzle3(){
  ...
  useEffect(() => {
    if(isComplete(puzzleState.success) && !notifyLocked){
      try{
        ...
        const solution = AES.decrypt(
          getPuzzleCipher(puzzleData),password
        ).toString(UTF8Enc);
        if(Boolean(solution)){
          lockNotifications();
          notify({
    ...
    puzzleState.success,lockNotifications,
    notify,notifyLocked,puzzleData,puzzleState.values,
    setUserSuccess
  ]);
              ...
              <PnPill
                checkFunc={
                  setSuccessHeader(
                    ...
                    bool => actOnPuzzleState(
                      {index,type:(bool) ? 'setSuccess' : 'setFailure'}
                    )
                  )
                }
