...
const useClasses = themeHook(
	...
	([cardPadding, minorSpacing]) => ({
		cell: {
			...
			minHeight: '76px',
			color: ({ solved }) =>
				solved === true ? '#357e37' : solved === false ? '#d32f2f' : 'rgba(0,0,0,.89)',
			padding: cardPadding,
			marginTop: ({ mTop }) => (mTop ? minorSpacing : 0),
			marginBottom: ({ mBot }) => (mBot ? minorSpacing : 0),
			boxShadow: ({ clearShadow }) =>
				!clearShadow ? '10px 10px 64px -17px rgba(0,0,0,0.75)' : null
		},
		rCell: {
			borderRadius: ({ order = true }) => (order ? '0px 8px 8px 0px' : '8px 0px 0px 8px')
		},
		lCell: {
			borderRadius: ({ order = true }) => (order ? '8px 0px 0px 8px' : '0px 8px 8px 0px')
		},
		pCell: {
...
export default function PnPill(props) {
	const { pChildren, nChildren, clearShadow, checkFunc } = props;
			...
			<Holder direction='row'>
				<Holder
					...
					{typeof pChildren === 'string' ? (
						<React.Fragment>
							...
							<Typography className={classes.type}>{pChildren}</Typography>
						</React.Fragment>
					) : (
						pChildren
					)}
				...
				<Holder
					...
					{typeof nChildren === 'string' ? (
						<React.Fragment>
							<Typography className={classes.type}>{nChildren}</Typography>
							{!clearShadow ? (
								<Button
									variant='outlined'
									className={classes.checkButton}
