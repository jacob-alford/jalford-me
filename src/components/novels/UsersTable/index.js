...
const fields = [
  ...
  {
    field: 'permissions',
    icon: color => <AssignmentInd style={{ ...styles.chipIcon, color: color }} />,
    label: 'Permissions'
  },
  {
    field: 'color',
    icon: color => <ColorLens style={{ ...styles.chipIcon, color: color }} />,
    label: 'Color'
  }
];

const checkParseInt = str => !Number.isNaN(parseInt(str, 10));
const getPerms = (users, uid) =>
  users.find(user => user.uid === uid).permissions.value.toString();
const permsHaveChanged = (currentPermsText, users, uid) =>
  parseInt(currentPermsText, 10) !== parseInt(getPerms(users, uid), 10);
...
function UsersTable(props) {
  ...
  const headerConfig = [
        ...
        if (currentPermsEdit === uid) {
          return (
            <Grid container direction='row' justify='center' alignItems='center'>
              <Grid item>
                <InputBase
                  style={styles.changePerms}
                  onChange={handlePermsTextChange}
                  value={currentPermsText}
                />
              </Grid>
              <Grid item>
                {permsHaveChanged(currentPermsText, users, uid) ? (
    ...
    },
    {
      ...
      ref: ['color'],
      sortable: false,
      transform: color => <ColorLens style={{ color: color }} />
    }
  ];
  return (
    <React.Fragment>
